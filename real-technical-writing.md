...
이런 경험 있으신가요? git log에서 본 긴 해시값이 무엇인지, merge conflict를 해결할 때 Git이 정확히 무엇을 비교하는지, git commit --amend를 쓰면 왜 커밋 ID가 바뀌는지 궁금했던 적이요. 이 글을 읽고 나면 이 모든 질문에 답할 수 있게 됩니다.
...
### Git 객체 탐색 도구: git cat-file

Git 객체를 살펴보기 위해 `git cat-file` 명령어를 사용합니다:
- `-t` 옵션: 객체의 타입 확인
- `-p` 옵션: 객체의 내용을 보기 좋게 출력 (pretty-print)
